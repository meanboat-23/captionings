...
class S2VT(torch.nn.Module):
    def __init__(self, vocab_size, frame_dim=4096, hidden_dim=1000, steps=80, video_embed_size=500, word_embed_size=500):
        ...
        self.lstm1 = torch.nn.LSTM(input_size=self.video_embed_size, hidden_size=self.hidden_dim, batch_first=True)
        self.word_out = torch.nn.Linear(hidden_dim, vocab_size)
        self.lstm2 = torch.nn.LSTM(input_size=self.word_embed_size + self.hidden_dim, hidden_size=self.hidden_dim, batch_first=True)
    ...
    def forward(self, video, caption=None):
        ...
        if self.training:
            word_vec = self.word_embedding(caption[:, :-1])
            padding_lstm1 = torch.zeros(batch, self.step - 1, self.video_embed_size).to(device)
